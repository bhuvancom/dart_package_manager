...
class PubDevRepositoryImpl implements PubDevRepository {
  ...
  Future<List<String>> searchPackages(String query, {int limit = 5}) async {
    final url =
        Uri.parse('https://pub.dev/api/search?q=${Uri.encodeComponent(query)}');
    final response = await _client.get(url);

    if (response.statusCode != 200) {
      throw Exception(
          'Failed to search packages. Status: ${response.statusCode}');
    }
  ...
  Future<PackageDetails?> getPackageDetails(String packageName) async {
    try {
      ...
      final description =
          pubspec['description'] as String? ?? 'No description provided.';
